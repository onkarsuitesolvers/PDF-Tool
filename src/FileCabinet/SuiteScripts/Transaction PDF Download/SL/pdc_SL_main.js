 ...
 *  action=getLookups    → Folder list → JSON               [pdc_mod_lookups]
 ...
 *  pdc_mod_lookups.js          ← getLookups action + fetchFolders
 *  pdc_mod_files.js            ← getFiles action (File Cabinet file search, by folder)
...
define(
  ...
  (runtime, log, serverWidget, lookups, files, fileDownloader, htmlBuilder) => {
  ...
  const onRequest = (ctx) => {
    ...
    log.debug({ title: 'PDC onRequest', details: 'action=' + action + ' | folder=' + (ctx.request.parameters.folder || '') });
